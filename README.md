...
* **VPC (10.10.0.0/16):** The main cloud network.
* **Public Subnets (DMZ):** This is where the managed AWS NAT Gateway lives. It has public connectivity and provides egress for private subnets.
* **Private Subnets (Secure Zone):** This is for internal servers (like the Active Directory) and office workloads. They don't have public IPs and use the NAT Gateway for internet egress.
* **Office workload on ECS Fargate:** Office simulation users are deployed as containers, not as EC2 instances.
* **Samba4 AD DC (EC2):** The main Active Directory domain controller (`corp.local`).
...
* **Automated DHCP Options:** The AWS network uses `AmazonProvidedDNS` for VPC-level name resolution.
...
* [x] **DHCP Options Set:** Using AWS-native `AmazonProvidedDNS` for VPC DNS.
* [ ] **PKI & LDAPS (Port 636):** Setting up a Root CA (Certificate Authority) to encrypt all Active Directory traffic and stop Man-in-Middle attacks.
* [ ] **Client-to-Site VPN:** Setting up WireGuard or OpenVPN so remote workers can safely connect to the company network from home.
* [ ] **Certificate-based authentication:** Using client certificates for stronger identity verification and future VPN / SSO scenarios.
...
* ❗ Change the S3 bucket name in `terraform/providers.tf` (in the `backend "s3"` block). ❗
